	...
	case MQ_OPENAI_RESPONSE:
		{
			...
			if (g_oairt.current_call) {
				warning("openai_rt: emit OPENAI_RESPONSE "
					"(%d)\n", UA_EVENT_OPENAI_RESPONSE);
				bevent_call_emit(UA_EVENT_OPENAI_RESPONSE,
					g_oairt.current_call,
				                "%s", response_json);
			}
			else {
				DEBUG_INFO("mqueue_handler: No active "
					"call for OpenAI response\n");
			}
...
int calls_queue_openai_response(const char *response_json)
{
	...
	if (calls_state.shutting_down) {
		DEBUG_INFO("calls_queue_openai_response: "
			"Ignoring during shutdown\n");
		return EINTR;
	}

	if (!calls_state.mq) {
		warning("openai_rt: calls_queue_openai_response: mqueue not "
			"initialized\n");
		return EINVAL;
	}
	...
	if (!response_json) {
		warning("openai_rt: "
			"calls_queue_openai_response: NULL response\n");
		return EINVAL;
	}

	/* Allocate memory for the JSON string (will be freed in mqueue
	 * handler) */
	err = str_dup(&json_copy, response_json);
	if (err) {
		warning("openai_rt: "
			"Failed to duplicate response JSON: %m\n", err);
		return err;
	}
	...
	if (err) {
		warning("openai_rt: "
			"Failed to queue OpenAI response: %m\n", err);
		mem_deref(json_copy);
		return err;
	}
