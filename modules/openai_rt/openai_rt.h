...
/* Maximum prompt length in bytes (including NUL). */
#define OPENAI_RT_PROMPT_MAX 32768
...
struct openai_rt {
	/* Configuration */
	char api_key[256];
	/* System prompt / instructions. Task prompts generated by the
	 * platform easily exceed 4 KiB; the buffer is sized so a config
	 * value is never silently cut (read_config warns if it would be). */
	char prompt[OPENAI_RT_PROMPT_MAX];
