...
struct ausrc_st {
	...
	/* Baresip's callback - we call this when we have audio */
	ausrc_read_h *rh;
...
struct auplay_st {
	...
	/* Baresip's callback - called when baresip has audio */
	auplay_write_h *wh;
	void *arg;                /* Baresip's argument */
	size_t sampc;             /* Samples per frame */
...
{
	/* We need both the playback driver (for receiving audio from
	 * call) and the source driver (for injecting audio back into
	 * call) to be ready */
	if (g_audio.play_st && g_audio.src_st && g_oairt.call_active &&
		g_audio.play_st->ready && g_audio.src_st->ready) {
		DEBUG_INFO("Both audio drivers ready and call active -"
			   " starting threads\n");
	...
	while (le) {
		...
		if (event && event->type == EVENT_CALL_START) {
			DEBUG_INFO("Found pending call start event, starting"
				   " threads\n");
...
int audio_init(void)
{
	...
	g_audio.injection_buffer = mem_alloc(
		g_audio.injection_buffer_size * sizeof(int16_t), NULL);
	if (!g_audio.injection_buffer) {
		warning("openai_rt: Failed to allocate injection buffer\n");
		goto cleanup;
	}

	DEBUG_INFO("Injection buffer initialized: %zu samples (%zu ms at"
		   " 24kHz)\n",
		   g_audio.injection_buffer_size,
		   g_audio.injection_buffer_size * 1000 / 24000);
	...
	g_audio.uplink_batch = mem_alloc(
		g_audio.uplink_batch_cap * sizeof(int16_t), NULL);
	...
	int err;
	mtx_t *inj_mtx = &g_audio.injection_buffer_mutex;
	err = mtx_init(inj_mtx, mtx_plain) != thrd_success;
	if (err != thrd_success) {
		warning("openai_rt: Failed to initialize injection buffer"
			" mutex: %d\n", err);
		goto cleanup;
	}
	...
	/* Initialize mutexes */
	err = mtx_init(&g_audio.read_queue_mutex, mtx_plain) != thrd_success;
	if (err != thrd_success) {
		warning("openai_rt: Failed to initialize read queue mutex:"
			" %d\n", err);
		goto cleanup;
	}

	err = mtx_init(&g_audio.write_queue_mutex, mtx_plain) != thrd_success;
	if (err != thrd_success) {
		warning("openai_rt: Failed to initialize write queue mutex:"
			" %d\n", err);
		goto cleanup;
	}

	err = mtx_init(&g_audio.event_queue_mutex, mtx_plain) != thrd_success;
	if (err != thrd_success) {
		warning("openai_rt: Failed to initialize event queue mutex:"
			" %d\n", err);
		goto cleanup;
	}

	/* Initialize condition variables */
	err = cnd_init(&g_audio.read_queue_cond) != thrd_success;
	if (err != thrd_success) {
		warning("openai_rt: Failed to initialize read queue condition:"
			" %d\n", err);
		goto cleanup;
	}

	err = cnd_init(&g_audio.write_queue_cond) != thrd_success;
	if (err != thrd_success) {
		warning("openai_rt: Failed to initialize write queue"
			" condition: %d\n", err);
		goto cleanup;
	}

	err = cnd_init(&g_audio.event_queue_cond) != thrd_success;
	if (err != thrd_success) {
		warning("openai_rt: Failed to initialize event queue"
			" condition: %d\n", err);
		goto cleanup;
	}

	DEBUG_INFO("Audio subsystem initialized with %zu byte buffers and"
		   " queues\n", g_audio.buffer_size);
	return 0;

cleanup:
	/* Clean up on error - mem_deref handles NULL safely, but mutexes/conds
	 * need to be destroyed if initialized */
	...
	/* Note: src_st and play_st are not allocated during init, so no need
	 * to free them here */
	return ENOMEM;
}
...
void audio_close(void)
{
	...
	DEBUG_INFO("Cleaning up audio system - read queue: %u, write queue:"
		   " %u, "
			   "event queue: %u\n",
	...
	/* Note: Only destroy if they were initialized - but we can't check, so
	 * we assume they were */
	/* If audio_init() failed early, these might not be initialized, but
	 * destroying uninitialized */
	/* mutexes/conds is undefined behavior. We'll rely on the fact that
	 * audio_init() should either */
...
static void queue_pcm_append(const int16_t *s16_data, size_t sampc)
{
	...
	if (err) {
		warning("openai_rt: Failed to build audio append"
			" message: %m\n", err);
		return;
	}
	...
	if (err) {
		...
		if (g_audio.audio_accumulated >= g_audio.commit_threshold) {
			if (g_oairt.session_cfg_applied)
				...
				DEBUG_INFO("Skipping commit - session not"
					   " ready yet\n");
			g_audio.audio_accumulated = 0;
		}
	}
...
static void flush_uplink_batch_internal(bool force)
{
	...
	if (!force &&
		g_audio.uplink_batch_len * sizeof(int16_t)
		< UPLINK_BATCH_BYTES)
		return;
...
/* Handle outgoing audio from OpenAI - convert from G711u and buffer for
 * injection */
void handle_outgoing_audio(const uint8_t *g711u_data, size_t len)
{
	...
	if (mbuf_get_left(g_audio.g711u_output_buffer) + len
	    > g_audio.buffer_size) {
...
static int read_from_injection_buffer(int16_t *samples, size_t max_samples)
{
	...
	for (size_t i = 0; i < samples_to_read; i++) {
		samples[i] =
			g_audio.injection_buffer[g_audio.injection_read_pos];
		g_audio.injection_read_pos =
			(g_audio.injection_read_pos + 1)
			% g_audio.injection_buffer_size;
		g_audio.injection_available--;
	}
	...
	if ((samples_to_read > 0) &&
	    (g_audio.injection_available
	     < g_audio.injection_buffer_size / 8)) {
...
static int ausrc_read_thread(void *arg)
{
	...
	DEBUG_INFO("Audio source thread started (srate=%u, ch=%u, ptime=%u,"
		   " sampc=%zu)\n",
	...
	while (st->run && st->ready) {
		...
		if (!g_oairt.call_active) {
			DEBUG_INFO("Audio source thread stopping - call"
				   " ended\n");
			break;
		}
		...
		if (audio_source_ready_for_injection()) {
			int got = read_from_injection_buffer(st->sampv,
				st->sampc);
		...
		if (have < st->sampc) {
			...
			if (frame) {
				...
				if (take > st->sampc - have)
					take = st->sampc - have;
				memcpy(st->sampv + have, frame->sampv,
					take * sizeof(int16_t));
				have += take;
...
static int auplay_write_thread(void *arg)
{
	...
	while (st->run && st->ready) {
		if (!g_oairt.call_active) {
			DEBUG_INFO("Audio playback drain thread stopping -"
				   " call ended\n");
			break;
		}
						  ...
						  void *arg)
{
	...
	DEBUG_INFO("Audio source configured: srate=%u, ch=%u, ptime=%u,"
		   " sampc=%zu\n",
		   st->prm.srate, st->prm.ch, st->prm.ptime,
		   st->sampc);

	/* Initialize mutex for thread safety */
	int err = mtx_init(&st->mutex, mtx_plain) != thrd_success;
	if (err != thrd_success) {
		warning("openai_rt: Failed to initialize audio source mutex:"
			" %d\n", err);
		mem_deref(st->sampv);
		mem_deref(st);
		return ENOMEM;
	...
	DEBUG_INFO("Audio source allocated, ready to provide audio"
		   " (call_active=%d)\n", g_oairt.call_active);
						   ...
						   void *arg)
{
	...
	int err = mtx_init(&st->mutex, mtx_plain) != thrd_success;
	if (err != thrd_success) {
		warning("openai_rt: Failed to initialize audio playback mutex:"
			" %d\n", err);
		mem_deref(st->sampv);
		mem_deref(st);
		return err;
	...
	DEBUG_INFO("Audio playback allocated, ready to receive audio"
		   " (call_active=%d)\n", g_oairt.call_active);
...
void audio_stop_threads(void)
{
	...
	if (g_audio.g711u_input_buffer) {
		int clear_result = safe_clear_buffer(
			g_audio.g711u_input_buffer, "input");
		if (clear_result) {
			warning("openai_rt: Failed to clear input"
				" buffer: %m\n", clear_result);
		}
	}
	if (g_audio.g711u_output_buffer) {
		int clear_result = safe_clear_buffer(
			g_audio.g711u_output_buffer, "output");
		if (clear_result) {
			warning("openai_rt: Failed to clear output buffer:"
				" %m\n", clear_result);
		}
	}
	...
	/* Don't clear WebSocket queue immediately - let it finish sending any
	 * pending messages */
	...
	if (g_audio.src_st && g_audio.src_st->ready) {
		...
		if (g_audio.src_st->thread) {
			...
			if (g_audio.src_st->run) {
				DEBUG_INFO("Audio source thread already"
					   " running\n");
			}
			else {
				/* Thread exists but not running - wait for it
				 * to finish and restart */
				DEBUG_INFO("Audio source thread exists but not"
					   " running, waiting to join...\n");
				thrd_join(g_audio.src_st->thread, NULL);
				g_audio.src_st->thread = 0;
			}
		}
		...
		if (!g_audio.src_st->thread) {
			...
			int thread_err = thread_create_name(
				&g_audio.src_st->thread, "openai_rt_src",
				ausrc_read_thread, g_audio.src_st);
			if (thread_err) {
				warning("openai_rt: Failed to restart audio"
					" source thread: %m\n", thread_err);
				g_audio.src_st->run = false;
			}
			else {
				info("openai_rt: Audio source thread started"
				     " successfully\n");
			}
		}
	}
	else {
		DEBUG_INFO("Audio source not ready for thread start: ready=%d,"
			   " src_st=%p\n",
	...
	if (g_audio.play_st && g_audio.play_st->ready) {
		...
		if (g_audio.play_st->thread) {
			...
			if (g_audio.play_st->run) {
				DEBUG_INFO("Audio playback thread already"
					   " running\n");
			}
			else {
				/* Thread exists but not running - wait for it
				 * to finish and restart */
				DEBUG_INFO("Audio playback thread exists"
					   " but not running, waiting to"
					   " join...\n");
				thrd_join(g_audio.play_st->thread, NULL);
				g_audio.play_st->thread = 0;
			}
		...
		if (!g_audio.play_st->thread) {
			...
			int thread_err = thread_create_name(
				&g_audio.play_st->thread, "openai_rt_play",
				auplay_write_thread, g_audio.play_st);
			if (thread_err) {
				warning("openai_rt: Failed to restart audio"
					" playback thread: %m\n", thread_err);
				g_audio.play_st->run = false;
				...
				info("openai_rt: Audio playback thread started"
				     " successfully\n");
			}
		}
	}
	else {
		DEBUG_INFO("Audio playback not ready for thread start:"
			   " ready=%d, play_st=%p\n",
				   g_audio.play_st
				   ? g_audio.play_st->ready : 0,
				   g_audio.play_st);
	}

	DEBUG_INFO("Audio threads restart attempted (src_running=%d,"
		   " play_running=%d)\n",
			   audio_threads_running(),
			   g_audio.play_st&&
				g_audio.play_st->thread&&
				g_audio.play_st->run);
}
...
{
	bool src_running = (g_audio.src_st&&
		g_audio.src_st->thread&&
		g_audio.src_st->run);
	bool play_running = (g_audio.play_st&&
		g_audio.play_st->thread&&
		g_audio.play_st->run);
...
int audio_queue_read_frame(const int16_t *sampv, size_t sampc, uint32_t srate,
	uint8_t ch)
{
...
int audio_queue_event(enum event_type type, void *data)
{
	...
	/* If this is a call start event and audio drivers are ready, start
	 * threads */
...
static void send_audio_commit(void)
{
	...
	if (!g_audio.response_created && g_oairt.call_active &&
		...
		err = model->build_response_create(g_oairt.prompt,
			&response_msg);

		if (!err && response_msg) {
			err = queue_message_to_openai(response_msg,
				str_len(response_msg), NULL, NULL);
			if (err) {
				warning("openai_rt: Failed to queue"
					" response.create message: %m\n", err);
			}
			else {
...
void audio_flush_accumulated(void)
{
	...
	if (g_audio.audio_accumulated > 0) {
		double ms = (double)g_audio.audio_accumulated / 2
			/ 24000.0 * 1000.0;
		info("openai_rt: flushing %.1f ms (%u bytes) of accumulated"
		     " audio\n",
...
int resize_injection_buffer(size_t new_size_samples)
{
	...
	int16_t *new_buffer = mem_alloc(new_size_samples * sizeof(int16_t),
		NULL);
	if (!new_buffer) {
		mtx_unlock(&g_audio.injection_buffer_mutex);
		warning("openai_rt: Failed to resize injection buffer to %zu"
			" samples\n", new_size_samples);
		return ENOMEM;
	}

	/* If we need to preserve existing data (shouldn't happen with our
	 * growth logic) */
	if (g_audio.injection_available > 0) {
		/* This is a safety check - with our growth logic, we shouldn't
		 * have data when growing */
		warning("openai_rt: Growing buffer with %zu samples still in"
			" use\n", g_audio.injection_available);

		/* Copy existing data to the beginning of the new buffer */
		int16_t *temp_buffer = mem_alloc(
			g_audio.injection_available * sizeof(int16_t),
			NULL);
		if (temp_buffer) {
			size_t temp_pos = 0;
			...
			while (temp_pos < g_audio.injection_available) {
				temp_buffer[temp_pos++] =
					g_audio.injection_buffer[old_pos];
				old_pos = (old_pos + 1)
					% g_audio.injection_buffer_size;
			}

			/* Copy back to beginning of new buffer */
			memcpy(new_buffer, temp_buffer,
				g_audio.injection_available * sizeof(int16_t));
	...
	info("openai_rt: Injection buffer resized to %zu samples (%.1f seconds"
	     " @ 24kHz)\n",
...
static void maybe_shrink_injection_buffer(void)
{
	...
	/* Only consider shrinking if we're well under capacity and buffer is
	 * large */
	if (g_audio.injection_available < g_audio.injection_buffer_size / 4 &&
		g_audio.injection_buffer_size
		> INJECTION_BUFFER_INITIAL_SIZE * 2) {

		size_t new_size = g_audio.injection_buffer_size
			/ INJECTION_BUFFER_GROWTH_FACTOR;
		...
		if (new_size >= INJECTION_BUFFER_INITIAL_SIZE) {
			...
			if (err) {
				warning("openai_rt: Failed to shrink injection"
					" buffer: %m\n", err);
			}
			return;
		}
...
int write_to_injection_buffer(const int16_t *samples, size_t sample_count)
{
	...
	size_t space = g_audio.injection_buffer_size
		- g_audio.injection_available;
	...
	if (sample_count > space) {
		...
		int err = resize_injection_buffer(new_size);
		if (err) {
			warning("openai_rt: Failed to grow injection buffer,"
				" dropping %zu samples\n", sample_count);
			return err;
		}
		...
		mtx_lock(&g_audio.injection_buffer_mutex);
		space = g_audio.injection_buffer_size
			- g_audio.injection_available;
	}

	/* Write contiguous, wrapping as needed */
	for (size_t i = 0; i < sample_count; i++) {
		g_audio.injection_buffer[g_audio.injection_write_pos]
			= samples[i];
		g_audio.injection_write_pos =
			(g_audio.injection_write_pos + 1)
			% g_audio.injection_buffer_size;
	}
