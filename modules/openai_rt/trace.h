 ...
 *
 * openai_rt is Sipfront's OWN test bot: it dials the voice bot under test and
 * drives the conversation as a simulated caller. This module accumulates a
...
/* Enable/disable capture. When disabled every add_* / write is a no-op, so the
 * default (non-task) voice-AI call behaves exactly as before. */
 ...
 * a no-op returning 0 when capture is disabled. Call from the RE main thread on
 * call close. */
