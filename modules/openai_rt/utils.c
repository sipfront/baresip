...
char *encode_audio_base64(const void *data, size_t len)
{
	...
	out_len = ((len + 2) / 3) * 4 + (len / 40) + 64;
	/*DEBUG_INFO("++++ out_len: %zu\n", out_len); */

	encoded = mem_zalloc(out_len, NULL);
	if (!encoded) {
		DEBUG_INFO("Failed to allocate base64 buffer of size"
			   " %zu\n", out_len);
		return NULL;
	}
	...
	if (err) {
		DEBUG_INFO("Failed to encode base64: %m"
			   " (input %zu bytes, buffer %zu)\n",
			   err, len, out_len);
		mem_deref(encoded);
		return NULL;
	}
...
size_t decode_audio_base64(const char *data, uint8_t **out)
{
	...
	if (!decoded) {
		DEBUG_INFO("Failed to allocate decode buffer of size"
			   " %zu\n", out_len);
		return 0;
	}
	...
	*out = decoded;
/*
	DEBUG_INFO("Decoded to %zu bytes"
...
int read_config(void)
{
	...
	conf_get_str(conf_cur(), "openai_rt_api_key",
		     g_oairt.api_key, sizeof(g_oairt.api_key));
	...
	conf_get_str(conf_cur(), "openai_rt_openai_model",
		     g_oairt.openai_model, sizeof(g_oairt.openai_model));
	conf_get_str(conf_cur(), "openai_rt_gemini_model",
		     g_oairt.gemini_model, sizeof(g_oairt.gemini_model));
	g_oairt.wait_for_greeting = true;
	conf_get_bool(conf_cur(), "openai_rt_wait_for_greeting",
		      &g_oairt.wait_for_greeting);
	g_oairt.temperature = 0.7f;
	{
		double temp_double = 0.7;
		conf_get_float(conf_cur(), "openai_rt_temperature",
			       &temp_double);
		g_oairt.temperature = (float)temp_double;
	}
	conf_get_str(conf_cur(), "openai_rt_voice",
		     g_oairt.voice, sizeof(g_oairt.voice));

	/* Conversation-trace capture (task-based voicebot assessment).
	 * Default off so plain voice / fixed-media calls behave exactly as
	 * before. When enabled we add input-audio transcription to the
	 * session and write a conversation-trace.json. */
	g_oairt.transcribe = false;
	conf_get_bool(conf_cur(), "openai_rt_transcribe",
		      &g_oairt.transcribe);
	conf_get_str(conf_cur(), "openai_rt_trace_dir",
		     g_oairt.trace_dir, sizeof(g_oairt.trace_dir));

	/* Gemini VAD config */
	g_oairt.gemini_vad_enabled = true;
	conf_get_bool(conf_cur(), "openai_rt_gemini_vad_enabled",
		      &g_oairt.gemini_vad_enabled);
	...
	if (!str_isset(g_oairt.backend))
		str_ncpy(g_oairt.backend, "openai_realtime",
			 sizeof(g_oairt.backend));

	/* Set default model names if not configured */
	if (!str_isset(g_oairt.openai_model))
		str_ncpy(g_oairt.openai_model, DEFAULT_OPENAI_MODEL,
			 sizeof(g_oairt.openai_model));
	if (!str_isset(g_oairt.gemini_model))
		str_ncpy(g_oairt.gemini_model, DEFAULT_GEMINI_MODEL,
			 sizeof(g_oairt.gemini_model));
	...
	if (str_casecmp(g_oairt.backend, "openai_realtime") == 0) {
		...
		warning("openai_rt: Unknown backend '%s',"
	...
	DEBUG_INFO("Config loaded - Backend: %s, API key: %s,"
		   " Prompt: %.50s%s, Tools: %s,"
		   " OpenAI model: %s, Gemini model: %s\n",
		   g_oairt.backend,
