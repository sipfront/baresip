	...
	.parameters_json =
		"{"
			...
			"\"properties\": {"
				"\"digits\": {"
					"\"type\": \"string\","
					"\"description\": \"String of DTMF "
					"digits to send (e.g., '123', "
					"'*9#', '1234*'). Valid characters: "
					"0-9, *, #, A-D\","
					"\"pattern\": \"^[0-9*#A-D]+$\""
				"}"
			"},"
		...
		"{"
			...
			"\"properties\": {"
				...
				"\"auth_username\": {"
					...
					"basic auth or token for bearer "
					"auth\""
				"},"
				"\"auth_password\": {"
					"\"type\": \"string\","
...
const struct ai_tool_call AI_TOOL_TRANSFER_CALL = {
	.name = "transfer_call",
	.description = "Transfer the active call to another "
		"destination (blind transfer)",
	.parameters_json =
		"{"
			"\"type\": \"object\","
			"\"properties\": {"
				"\"destination\": {"
					"\"type\": \"string\","
					"\"description\": \"SIP URI or "
					"phone number to transfer the call "
					"to (e.g., 'sip:agent@example.com' "
					"or '+43123456789')\""
				"}"
			"},"
...
/* General-purpose observable-event tool: lets the simulated caller record any
 * specific fact or value it heard (a confirmation number, a quoted price, an
 * appointment time, ...) into the conversation trace so the downstream task
 * evaluator can verify outcomes structurally, not just from the transcript.
 * Performs no side effect on the call -- the event is captured under its
 * caller-supplied label and acknowledged. Enabled only when named in
 * openai_rt_tool_calls. */
const struct ai_tool_call AI_TOOL_RECORD_EVENT = {
	.name = "record_event",
	.description = "Record a specific fact or event stated during the "
		"call (e.g. a confirmation/reference number, a quoted "
		"price, an appointment time) so the test can verify "
		"it. Provide a short, stable label describing what the "
		"value is.",
	.parameters_json =
		"{"
			"\"type\": \"object\","
			"\"properties\": {"
				"\"label\": {"
					...
					"identifier of what is being recorded "
					"(e.g. 'confirmation_number', "
					"'quoted_price', 'appointment_time')\""
				"},"
				"\"value\": {"
					"\"type\": \"string\","
					"\"description\": \"The value exactly "
					"as stated by the agent (include "
...
static int openai_build_audio_append(const char *base64_audio,
	char **json_msg);
static int openai_build_response_create(const char *instructions,
	char **json_msg);
static int openai_build_function_call_output(const char *call_id,
	const char *name,
...
bool ai_model_is_tool_enabled(const char *tool_name, const char *enabled_tools)
{
	...
	while (*p) {
		...
		if (strncmp(p, tool_name, tool_name_len) == 0) {
			/* Check if this is a complete match (followed by
			 * comma, space, or end) */
...
static int openai_build_session_update(const char *prompt, char **json_msg)
{
	...
	/* Optionally enable input-audio transcription so we capture the
	 * AGENT-under-test side of the conversation (the model's own speech
	 * already surfaces via response.output_audio_transcript.done). Off by
							   ...
							   void *cb_arg)
{
	...
	const char *type = get_json_string_field(root,
		"type",
		"message parser");
	if (!type) {
	...
	if (strcmp(type, "response.output_audio.delta") == 0) {
		const char *delta = get_json_string_field(root,
			"delta",
			"audio_delta");
		...
		struct json_object *item_obj = get_json_object_field(root,
			"item",
			"output_item.done");
		if (item_obj) {
			const char *item_type = get_json_string_field(item_obj,
				"type",
				"output_item.done");
			if (item_type && strcmp(item_type,
				"function_call") == 0) {
				const char *name = get_json_string_field(
					item_obj,
					"name",
					"function_call");
				const char *call_id = get_json_string_field(
					item_obj,
					"call_id",
					"function_call");
				const char *arguments = get_json_string_field(
					item_obj,
					"arguments",
					"function_call");
		...
		== 0) {
		/* Transcription of the input audio = the far end (bot under
		 * test) speaking to us. Only emitted when input transcription
		 * is enabled (openai_rt_transcribe). */
		...
		/* Transcript of our own generated audio = this endpoint (our
		 * model). The two event names cover the GA and beta Realtime
		 * API namings. */
		struct json_object *t = NULL;
		if (json_object_object_get_ex(root, "transcript", &t) &&
		    json_object_is_type(t, json_type_string)) {
			trace_add_turn(TRACE_ROLE_SELF,
				json_object_get_string(t));
		}
	}
	else if (strcmp(type, "error") == 0) {
		struct json_object *error_obj = get_json_object_field(root,
			"error",
			"error");
		if (error_obj) {
