...
static void notify_event_emit(struct ua *ua, const struct sip_msg *msg)
{
	...
	if (msg->mb && mbuf_get_left(msg->mb) > 0) {
		re_sdprintf(&buf, "%b", mbuf_buf(msg->mb),
...
static int cmd_subscribe(struct re_printf *pf, void *arg)
{
	...
	if (!line || *line == '\0') {
		re_hprintf(pf,
			   "Usage: /subscribe <target> <event> <expires>\n");
		return 0;
	}
	...
	/* sipevent_subscribe: sock, uri, from_name, from_uri, event, id,
	 * expires, cuser, routev/routec, auth, aref, forkh/notifyh/closeh,
	 * fmt */
	int err = sipevent_subscribe(&sub->sub,
				     ...
				     ua_cuser(ua),
				     routev, routev[0] ? 1 : 0,
				     auth_handler, ua_account(ua),
				     false,
				     NULL, notify_handler, close_handler, sub,
				     "%s", hdrstr);
